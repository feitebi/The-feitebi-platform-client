...
var index = 0;
GetUrl('https://coinmarketcap.com/all/views/all/',data=>{ //把函数传入
	//console.log(data) //默认打印gbk
	var html = gbk.toString('utf-8',data)
	let DOM = new JSDOM(html);
	let document = DOM.window.document;
	//提取积分数据
	var title = document.querySelectorAll('td.col-symbol');
	var content = []
	for (item of title) {
	...
	//tips:诸如'.tm-price'这类敏感重要数据 又存在302 所以此处会报错
	fs.writeFile('array.html',content,'utf8')// 把参数写在html文件里
	console.log('爬取成功')
})
